...
const STORY_CATEGORIES = [
    'Ngôn tình', 'Học đường', 'Sủng', 'Tiên hiệp', 'Huyền huyễn',
    'Xuyên không', 'Trọng sinh', 'Hệ thống', 'Linh dị', 'Ma pháp',
    'Kiếm hiệp', 'Phiêu lưu', 'Quân sự', 'Trinh thám', 'Kinh dí',
    'Đô thị', 'Cung đấu', 'Hài hước', 'Cổ đại', 'Đồng nhân', 'Ngự thú'
];

// Hàm tạo và render category tags
function renderCategoryTags(selectedCategories = []) {
    ...
    STORY_CATEGORIES.forEach(category => {
        ...
        tag.type = 'button';
        tag.className = 'tag' + (selectedCategories.includes(category) ? ' selected' : '');
        ...
        tag.addEventListener('click', (e) => {
            ...
            if (tag.classList.contains('selected')) {
                ...
                if (selectedCount < 5) {
                    ...
                    alert('Bạn chỉ có thể chọn tối đa 5 thể loại');
                    return;
                }
            }
            
            // Cập nhật số lượng đã chọn
            const count = container.querySelectorAll('.tag.selected').length;
            document.getElementById('categoryCount').textContent = `Đã chọn: ${count}/5`;
        });
    ...
    if (storyId) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Render tags với các thể loại đã chọn
                renderCategoryTags(Array.isArray(story.category) ? story.category : []);
            }
        } catch (e) {
...
async function saveStory() {
    ...
    // Lấy các thể loại đã chọn từ tags
    const selectedTags = document.querySelectorAll('#categoryTags .tag.selected');
    const category = Array.from(selectedTags).map(tag => tag.textContent.trim());
