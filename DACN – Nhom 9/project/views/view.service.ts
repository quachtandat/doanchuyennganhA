...
export class ViewService {
  ...
  */
  ...
  async approveChapter(id: string): Promise<boolean> {
    const res = await this.chapterModel.findByIdAndUpdate(id, { $set: { status: 'published' } }).exec();
    return res !== null;
  }

  /** Reject a chapter (set status to 'rejected') */
  async rejectChapter(id: string): Promise<boolean> {
    const res = await this.chapterModel.findByIdAndUpdate(id, { $set: { status: 'rejected' } }).exec();
    return res !== null;
  }

  /** Delete a chapter */
  async deleteChapter(id: string): Promise<boolean> {
    const res = await this.chapterModel.findOneAndDelete({ _id: id }).exec();
    return res !== null;
  }
