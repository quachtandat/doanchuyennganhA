...
export class AuthorController {
  ...
  async overview(@Request() req) {
    ...
    const revenueAgg = await this.purchaseModel.aggregate([
      { $match: { storyId: { $in: storyIds.map(id => id.toString()) }, status: 'completed' } },
  ...
  async revenueStatsMonthly(@Request() req) {
    ...
    const stats = await this.purchaseModel.aggregate([
      { $match: { storyId: { $in: storyIds.map(id => id.toString()) }, status: 'completed', purchaseAt: { $gte: twelveMonthsAgo } } },
      {
        $group: {
    ...
    const stats = await this.readingHistoryModel.aggregate([
      ...
      },
      {
        $lookup: {
          ...
          let: { sid: '$_id' },
          pipeline: [
            { $match: { $expr: { $and: [ { $eq: ['$storyId', { $toString: '$$sid' }] }, { $eq: ['$status', 'completed'] } ] } } },
            { $group: { _id: null, totalCoins: { $sum: '$priceCoins' } } },
          ],
          as: 'purchases',
        },
